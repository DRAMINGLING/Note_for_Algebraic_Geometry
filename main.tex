...
\begin{proof}
    ...
    \par
    For all $\alpha = (\beta, t)$ with $\beta\in V$, we get that $T_{\alpha}X$ is defined by equations $S \cup \{\sum_{j} \frac{\partial G}{\partial x_{j}}(\alpha)x_{j}\}$,
    where $S$ is the set of defining equations of $T_{\beta}Y$.
    Note that each fiber of $\varphi$ is isomorphic to $\mathbb{A}_{k}^{1}$, 
\end{proof}
\begin{example}
