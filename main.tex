...
\begin{proof}
    ...
    As $X$ os connected projective variety, by Corollary \ref{Corollary 3.2}, $\frac{G}{F}\big{|}_{X}$ is constant.
...
\begin{proof}
    ...
    Note that $C^{0}(X) \cap C^{0}(Y) = (C^{0}(X) \times C^{0}(Y)) \cap \Delta_{\mathbb{A}_{k}^{n + 1}}$ and by Corollary \ref{Corollary 4.5}, $\dim (C^{0}(X) \times C^{0}(Y)) = \dim X + \dim Y + 2$,
...
\begin{proof}
    ...
    where $d = \dim Y$.
    Note that $\{y\}$ is a connected component of $V \cap V(F_{1}, \cdots, F_{d})$, we get $X_{x} = \varphi^{-1}(x)$ is a union of some irreducible components of $\varphi^{-1}(V) \cap V(\varphi^{\ast}F_{1}, \cdots, \varphi^{\ast}F_{d})$.
    \par
    By Krull's Principal Ideal Theorem, each irreducible component has dimension $\ge \dim X - d$.
...
\begin{proof}
    ...
    By Corollary \ref{Corollary 4.7}, there exists nonempty open subset $U \subseteq Y$ such that for all $y\in Y$,
...
\begin{proof}
    ...
    \begin{equation}
        \begin{split}
            \dim T_{z}\varphi^{-1}(Z) & \le \dim T_{\varphi(z)}Z + \dim_{k} (\ker T_{z}\varphi) \\
            & = \dim Z + \dim X - \dim Y \\
            & = \dim X - r
        \end{split}
    \end{equation}
