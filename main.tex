...
\begin{definition}
    A line bundle is equivalence class of a collection of data 
    \begin{itemize}
    ...
    \end{itemize}
    under the equivalent relation $(\{U_{i}\}, g_{ij}) \sim (\{V_{i'}\}, g_{i'j'}')$ if there exist regular functions $s_{ii'}: U_{i} \cap V_{i'} \rightarrow k^{\ast}$ such that $g_{i'j'}'s_{ii'} = g_{ij}s_{jj'}$ for all $i, j, i', j'$.
\end{definition}
\begin{remark}
    More precisely, such an equivalence would one-to-one correspond to an isomorphism class of our original definition instead of just a morphism.
\end{remark}
...
\begin{proposition}
    ...
    Assume $s_{0}, \cdots, s_{i}\in \Gamma(X, L)$, where $s_{i} = \{s_{ik}\}$
...
\subsection{Divisors}
\label{subsection:Divisors_Divisors}
