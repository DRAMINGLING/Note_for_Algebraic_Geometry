...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{pmatrix}
    \end{equation}
    where $M$ represents the defining equations in $S$.
...
\begin{theorem}[\textbf{\emph{Generic Smoothness}}]
    ...
    where $\varphi^{-1}(V) \cap X^{\smooth}$ is the schemetic intersection i.e. the ideal sheaf would generated union of the original two.
    \label{Theorem 5.4}
\end{theorem}
\begin{example}
    Assume $\characteristic k = 0$.
...
\end{example}
\begin{proof}
    We may assume that $X$ and $Y$ are both smooth.
    Let $Z = \{x\in X \big{|} \}$ which is closed in $X$.
    For all irreducible component $Y'$ of $\overline{\varphi(Z)}$,
    we can pick an irreducible component $X'$ of $Z$ such that $\varphi\big{|}_{X'}$ is dominant.
    ...
    Hence $\dim Y' = \dim V' \le \dim U' = \dim X' < d$.
    Take $V = Y \setminus Y'$.
\end{proof}
\end{document}