...
\begin{proof}
    ...
    \par
    Note that $\delta_{X}$ is isomorphic to $X$ as varieties and is defined by $\{f(x_{0}, x_{1}, x_{2}), f(y_{0}, y_{1}, y_{2}), x_{0}y_{1} - x_{1}y_{0}, x_{0}y_{2} - x_{2}y_{0}, x_{1}y_{2} - x_{2}y_{1}\}$,
    we get that $\delta_{X}$ is an irreducible subvariety of $X \times X$ of codimension $1$.
    To better interpret $\mathcal{L}(\Delta_{X})$ as transition functions, consider $\Delta_{X} \hookrightarrow \cup_{i} (U_{i} \times U_{i}') \hookrightarrow X \times X$,
    where $U_{i} = X \cap D_{+}(x_{i})$ and $U_{i}' = X \cap D_{+}(y_{i})$ in $\mathbb{P}_{k}^{2}$.
\end{proof}
