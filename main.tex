...
\begin{remark}
    ...
    \par
    In particular, the Segre embedding $(\mathbb{P}_{k}^{n} \times \mathbb{P}, \text{ product topology}) \rightarrow (\mathbb{P}_{k}^{mn + m + n}, \text{ Zariski topology})$ is a continuous map.
...
\begin{proof}
    ...
    If we take $U = X \setminus (\cup Z')$, where $Z'$ varies all irreducible components of $X(\dim X - \dim Y + 1)$ except $Z$,
...
\begin{definition}
    ...
    where multiplication $\cdot: X \times X \rightarrow X$, inverse $\iota: X \rightarrow X$ and identity $e: \spec k \rightarrow X$ satisfy group structure.
